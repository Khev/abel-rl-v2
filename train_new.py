#!/usr/bin/env python
"""
train.py – one script for PPO, A2C, DQN-PER *or* custom SAC *or* DQN-XER.
...
# --- Episode‐Level Success Buffer and Recorder ---
from collections import deque
import random

class EpisodeSuccessBuffer:
    """FIFO buffer of solved episodes (each is a list of transitions)."""
    def __init__(self, capacity: int = 50):
        self.episodes: deque[list] = deque(maxlen=capacity)
...
class EpisodeRecorder(gym.Wrapper):
    """
    Wraps an Env to record transitions of each episode,
    and pushes solved episodes into an EpisodeSuccessBuffer.
    """
    def __init__(self, env, success_buffer: EpisodeSuccessBuffer):
        super().__init__(env)
    ...
    def step(self, action):
        ...
        self.current_episode.append((prev_obs, action, reward, obs, done, info))
        if done and info.get("is_solved", False):
            self.success_buffer.add_episode(self.current_episode)
        self.last_obs = obs
...
class SolveLoggerCallback(BaseCallback):
    ...
    then, for 'dqn-xer', replays stored successful transitions into the buffer,
    runs one deterministic evaluation episode in a separate env and
    stops training if it also solves.
    """
    def __init__(self, eval_env, success_buffer, algo, verbose: int = 0):
    ...
    def _on_step(self) -> bool:
        ...
        for info in infos:
            if info.get("is_solved", False):
                ...
                print(f"{self.algo}: Solved at T = {self.num_timesteps}: "
                      f"{env0.main_eqn} ==> {info['lhs']} = {info['rhs']}")

                if self.algo == 'dqn-xer':
                    # for episode in self.success_buffer.episodes:
                    #     for (obs, action, reward, next_obs, done, info_dict) in episode:
                    #         # Note: must include the original info dict
                    #         self.model.replay_buffer.add(obs, next_obs, action, reward, done, [info_dict])  # Wrap in list
                    if self.success_buffer.episodes:  # Ensure there's at least one
                        episode = self.success_buffer.episodes.pop()  # Deque supports indexing; -1 is newest
                        for (obs, action, reward, next_obs, done, info_dict) in episode:
                            self.model.replay_buffer.add(obs, next_obs, action, reward, done, [info_dict])

                        


                # Deterministic evaluation for early stopping
...
def main(args):
    ...
    # create success buffer for DQN-XER
    success_buffer = EpisodeSuccessBuffer(capacity=50)

    eval_env = Monitor(make_env(args.env_id))
    eval_cb  = EvalCallback(eval_env, eval_freq=args.eval_freq,
                            deterministic=True, verbose=0)
    solve_logger = SolveLoggerCallback(eval_env, success_buffer, args.algo)
    ...
    elif args.algo == 'dqn-xer':
        raw_env = make_env(args.env_id)
        env     = Monitor(EpisodeRecorder(raw_env, success_buffer))
        model = DQN(
            "MlpPolicy", env,
            buffer_size            = 100_000,
            replay_buffer_class    = PERBuffer,
...
if __name__ == "__main__":
    ...
    p.add_argument("--algo",      default="ppo",
                   choices=["ppo", "a2c", "dqn", "dqn-per", "dqn-xer", "sac"])
    ...
    algos = ["ppo", "a2c", "dqn", "dqn-per", "dqn-xer"]
    algos = ["dqn-per", "dqn-xer"]
    n_trials = 3
    results = {}
    for key in algos:
        trials = []
        for trial in range(n_trials):
            args.algo = key
            args.seed = trial + 1  # Vary seed for each trial
            Tfirst, Tstop = main(args)
            trials.append((Tfirst, Tstop))
        results[key] = trials
    ...
    for key, trials in results.items():
        ...
        success_rate = len(tsolves) / n_trials
        ...
        print(f'  Tsolve = {mean_tsolve:.2f} ± {std_tsolve:.2f}')
        print(f'  Tconverge = {mean_tconverge:.2f} ± {std_tconverge:.2f}')

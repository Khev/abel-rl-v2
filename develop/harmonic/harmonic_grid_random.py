...
class HarmonicLayerL1(nn.Module):
    ...
    def forward(self, x):
        x_embedded = self.embedding(x.argmax(dim=1))
        dist = torch.cdist(x_embedded.unsqueeze(0), self.weights.unsqueeze(0), p=1).squeeze(0)
        inv_dist = 1 / (dist**self.n + 1e-8)
        probs = inv_dist / inv_dist.sum(dim=1, keepdim=True)
        return probs
...
def train_ensemble(policy, env, optimizer_class, optimizer_args, episodes=10000, baseline_name="", n_ensemble=1, log_interval=100, test_env=None, test_eval_episodes=10):
    ensemble_results = []
    # Also record accuracy data per ensemble member:
    acc_data = []  # List of dicts per ensemble member, each dict has keys: 'episodes', 'train_acc', 'test_acc'
    ...
    for i in range(n_ensemble):
        ...
        for episode in range(episodes):
            ...
            reinforce_loss = -sum([lp * total_return for lp in log_probs])
            optimizer.zero_grad()
            reinforce_loss.backward()
            optimizer.step()
    ...
    # Compute mean, min, max over ensemble for each log interval
    # Assume that all ensemble members logged the same episode numbers.
    ...
    # Determine the best model based on the average return over the last 10% of episodes.
    best_model = None
    best_avg_return = -float('inf')
    best_losses = None
    last_portion = episodes // 10
...
def plot_accuracy_curves(accuracy_data):
    ...
    for ax, policy in zip(axs, policies):
        ...
        # Plot train accuracy curve with fill
        ...
        # Plot test accuracy curve with fill
...
if __name__ == "__main__":
    ...
    parser.add_argument("--n_ensemble", type=int, default=5, help="Number of ensemble members per policy")
    ...
    train_possible_starts = [(0, 0), (0, 1), (1, 0), (1, 1),
                            (4, 0), (4, 1), (3, 2)]  # Three extra distinct starts
    train_possible_goals  = [(3, 3), (3, 4), (4, 3), (4, 4),
                            (4, 2), (0, 4), (2, 4)]  # Three extra distinct goals

    test_possible_starts  = [(0, 0), (0, 1), (1, 1), (1, 2),
                            (3, 4), (4, 4), (2, 3)]   # Three extra distinct starts
    test_possible_goals   = [(3, 3), (4, 3), (4, 4), (4, 2),
                            (0, 0), (0, 1), (1, 0)]   # Three extra distinct goals
    ...
    for label, PolicyClass, filename in policies_to_train:
        ...
        policy_instance = PolicyClass(input_dim=input_dim, output_dim=output_dim, embedding_dim=embedding_dim)

...
def cyc_expansions(eqn_str, letter_list, n_exp=5):
    ...
    if not placeholders:
        return [eqn_str]*n_exp  # No placeholders => just return eqn_str repeated
...
def generate_data(eqns, max_level=10, outdir="equation_templates/shallow"):
    """
    For each level from 0..(max_level-1),
    - eqn_set = eqns[:level+1]
    - produce 5 expansions for each eqn in eqn_set using train_letters => train_eqns
    ...
    for lvl in range(max_level):
        ...
        for eqn_template in eqn_set:
            train5 = cyc_expansions(eqn_template, train_letters, n_exp=5)
            test5  = cyc_expansions(eqn_template, test_letters, n_exp=5)
            
            train_all.extend(train5)
            test_all.extend(test5)
        
        level_dir = os.path.join(outdir, f"level{lvl}")
        os.makedirs(level_dir, exist_ok=True)

        # Save to .txt instead of .npy
...
if __name__ == "__main__":
    generate_data(eqns, max_level=10, outdir="equation_templates/shallow")

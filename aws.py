...
- AWS adaptations: checkpointing with uploads to S3, basic resumption from latest S3 checkpoint (for 'ppo' algo only; ICM state not saved/resumed).
"""
...
import matplotlib
matplotlib.use('Agg')  # Non-interactive backend for server/AWS
...
class TrainingLogger(BaseCallback):
    def __init__(self, log_interval, eval_interval, save_dir, eval_env, algo_name, verbose=1, s3_bucket=None, s3_prefix=None):
    ...
    def _on_training_end(self):
        ...
        if self.s3_bucket:
            s3_path = os.path.join(self.s3_prefix, "learning_curves.csv") if self.s3_prefix else "learning_curves.csv"
            boto3.client('s3').upload_file(csv_path, self.s3_bucket, s3_path)
            log(f"Uploaded learning curves to S3: s3://{self.s3_bucket}/{s3_path}")
...
class UploadCheckpointCallback(BaseCallback):
    def __init__(self, bucket, s3_prefix, upload_freq, save_dir, verbose=1):
        super().__init__(verbose)
        self.bucket = bucket
        self.s3_prefix = s3_prefix
    ...
    def _on_step(self) -> bool:
        if self.n_calls % self.upload_freq == 0:
            # Find all checkpoints and upload the latest
            checkpoints = sorted(glob.glob(os.path.join(self.save_dir, '*_steps.zip')), key=lambda p: int(os.path.basename(p).split('_')[-2]))
            if checkpoints:
                latest_local = checkpoints[-1]
                s3_key = os.path.join(self.s3_prefix, os.path.basename(latest_local)) if self.s3_prefix else os.path.basename(latest_local)
                boto3.client('s3').upload_file(latest_local, self.bucket, s3_key)
                log(f"Uploaded latest checkpoint to S3: s3://{self.bucket}/{s3_key}")
        return True
...
def main(args):
    ...
    # Resumption logic: download latest checkpoint from S3 if available (for 'ppo' only)
    model = None
    if s3_bucket and algo != 'ppo-gnn':
        s3_resource = boto3.resource('s3')
        bucket_obj = s3_resource.Bucket(s3_bucket)
        checkpoint_objs = [obj for obj in bucket_obj.objects.filter(Prefix=s3_prefix) if obj.key.endswith('_steps.zip')]
        if checkpoint_objs:
            # Sort by steps
            checkpoint_objs.sort(key=lambda obj: int(obj.key.split('_')[-2]))
            latest_obj = checkpoint_objs[-1]
            local_checkpoint = os.path.join(save_dir, os.path.basename(latest_obj.key))
            latest_obj.download_file(local_checkpoint)
            log(f"Downloaded and resuming from S3 checkpoint: s3://{s3_bucket}/{latest_obj.key}")
            model = MaskablePPO.load(local_checkpoint, vec_env)
    ...
    if s3_bucket:
        # Add checkpoint saving and uploading
        checkpoint_callback = CheckpointCallback(
            save_freq=10**5,
            save_path=save_dir,
            name_prefix=f"{algo}-nenvs-{n_envs}"
        )
        ...
        callbacks.append(upload_callback)
...
if __name__ == "__main__":
    ...
    p.add_argument("--Ntrain",            type=int, default=10**3, help="total PPO timesteps (do NOT multiply by n_envs)")
    p.add_argument("--n_envs",            type=int, default=4,     help="number of parallel environments")
    p.add_argument("--trials",            type=int, default=1,     help="number of trials per scenario (different seeds)")
    ...
    p.add_argument("--s3_bucket",         type=str, default=None, help="S3 bucket name for checkpointing and results")
    p.add_argument("--s3_prefix",         type=str, default="",   help="S3 key prefix (e.g., 'benchmarks/')")
    args = p.parse_args()
    ...
    for algo_name, use_cur in scenarios:
        ...
        for t in range(args.trials):
            ...
            # Set per-trial S3 prefix
            trial_s3_prefix = os.path.join(args.s3_prefix, scen, f"n_envs_{args.n_envs}", f"trial_{t+1}")
            run_args.s3_prefix = trial_s3_prefix
    ...
    if args.s3_bucket:
        s3_csv_key = os.path.join(args.s3_prefix, f"benchmark_summary_nenvs_{args.n_envs}.csv")
        boto3.client('s3').upload_file(out_csv, args.s3_bucket, s3_csv_key)
        log(f"Uploaded aggregated summary to S3: s3://{args.s3_bucket}/{s3_csv_key}")
    ...
    # plt.show()  # Commented out for server/AWS
    log(f"Saved combined figure → {summary_fig_path}")

    if args.s3_bucket:
        s3_fig_key = os.path.join(args.s3_prefix, f"summary_learning_curves_nenvs_{args.n_envs}.png")
        boto3.client('s3').upload_file(summary_fig_path, args.s3_bucket, s3_fig_key)
        log(f"Uploaded combined figure to S3: s3://{args.s3_bucket}/{s3_fig_key}")